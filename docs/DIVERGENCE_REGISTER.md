...
| **Evidence** | `data/BTCUSDT_1h_gaps.csv` sums to 122 missing bars across 27 blocks; the report's 121 differs by exactly the boundary bar. Verified directly: `ls data/raw/` is empty; `spot_klines_btc.py` (651 lines) sits at the repository root |
...
| **D33** | Drop the `2026-08-01T00:00` boundary bar at ingest, then re-emit the report; correct every `data/raw/` path to `data/`, or move the four artifacts into `data/raw/` | Stage 2 onwards; Table 1 |
