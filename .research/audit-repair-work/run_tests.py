"""Run each existing test file in a fresh process to bound local RAM retention."""
import json
import subprocess
import sys
import time
...
logs = root / '.research/temp/pytest-audit'
...
for path in files:
    ...
    results.append(row)
    (root / '.research/audit-repair-work/pytest_results.json').write_text(
