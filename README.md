...
| Launcher | `notebooks/iTransformer.ipynb` — **self-contained**: it materialises the package from its own cells, so only the data Dataset is attached (`D54`). Generated by `tools/build_notebook.py`, never hand-edited |
