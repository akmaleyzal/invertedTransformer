...
| Measured | 534 iTransformer runs in **2.31 h** on 2 × T4, ~30 s per run — against a 60–100 s / 10–20 h estimate (`D57`). The 150 baseline runs are not yet measured on a T4; on CPU PatchTST is ~16× iTransformer per run, so expect two sessions |
