...
| Grid | **684 runs** — 534 iTransformer (main, uniform-attention, falsification, horizon sweep) + 150 baselines (ridge 60, DLinear 45, PatchTST 45). Not 837: 48 sweep cells share a `run_id` with the main grid (`D53e`). Not 789 either: that counted ARIMA, LSTM and the naive variants, deferred with a written reason in §7 (`D56`) |
| Hardware | Kaggle 2 × T4, **both used** (`D68`), 12 h per session, 30 GPU-h per week |
| Launcher | `notebooks/iTransformer.ipynb` — **self-contained**: thirteen cells define the package in the kernel namespace, so nothing is imported and only the data Dataset is attached (`D54`, `D58`). Generated by `tools/build_notebook.py`, never hand-edited |
| Measured | 534 iTransformer runs in **2.31 h** on 2 × T4, ~30 s per run — against a 60–100 s / 10–20 h estimate (`D57`). The full 894-run manifest ran in **7.79 h on one device**; `D68` puts both to work, and the manifest is now **969** |
